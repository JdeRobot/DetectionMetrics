...
public:
    ...
    DarknetDetections process(image& im , float threshold = 0.24);
    DarknetDetections process(const cv::Mat& im, float threshold = 0.24);
    std::string processToJson(const cv::Mat& im, float threshold = 0.24);
};
